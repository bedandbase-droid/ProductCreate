...
def build_prompt(style_name, style_text, room_type, labels, extra):
    ...
    if room_type.startswith("Outdoor Area"):
        outdoor = (
            ...
            "patio, veranda, deck, courtyard, terrace, poolside or garden area. Choose what fits naturally."
        )
...
If several furniture pieces are supplied, include all of them as separate real objects in one believable scene.
Do not merge their designs or borrow features from one product for another.
Only make normal photographic adjustments required for realistic placement: perspective, scale,
floor contact, realistic shadows and sensible overlap.
...
lamps, tables, accessories and supporting decor. The room should look professionally styled and believable.
...
Use clean, bright, balanced photographic lighting. Do not underexpose the furniture, crush shadow detail,
create strong colour casts on the product, or use dramatic darkness that hides fabric texture and construction detail.
...
if 1 <= len(uploaded_files) <= 3:
    ...
    for index, (uploaded, column) in enumerate(zip(uploaded_files, columns)):
        with column:
            ...
            labels.append(st.text_input(f"Furniture {index + 1} name / role", value=default_name, key=f"label_{index}"))
...
if style_name == "Custom":
    style_text = st.text_area("Describe the style direction", placeholder="Give a loose design direction. The AI will create the actual decor.")
else:
...
extra = st.text_area(
    ...
    placeholder="Examples: Put the chair beside the couch. Use a garden view. Make the room more premium. Do not include a coffee table.",
    height=110,
)
...
if st.button("➕ Add image to background queue", type="primary", use_container_width=True, disabled=not (api_key and valid_uploads and valid_setup)):
    submit_job(api_key, uploaded_files, prompt, style_name, room, model, quality, size)
    st.success("Added to the queue. You can immediately prepare the next image while this one generates.")
        ...
        with st.expander(f"{job['status']} — {job['style']} / {job['room']}", expanded=job["status"] in {"Generating", "Failed"}):
            ...
            if job["status"] == "Generating":
                ...
                if output_path and Path(output_path).exists():
                    ...
                    st.download_button("Download image", image_bytes, file_name=Path(output_path).name, mime="image/jpeg", key=f"download_{job['id']}")
